...
class Explore_Theme_Review_Notice {
	...
	public function __construct() {
		...
		add_action( 'after_setup_theme', array( $this, 'explore_theme_rating_notice' ) );
		add_action( 'switch_theme', array( $this, 'explore_theme_rating_notice_data_remove' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'explore_theme_rating_notice_enqueue' ) );
	 ...
	 */
	public function explore_theme_rating_notice() {

		// Set the installed time in `explore_theme_installed_time` option table.
		$option = get_option( 'explore_theme_installed_time' );
		if ( ! $option ) {
			update_option( 'explore_theme_installed_time', time() );
		}

		add_action( 'admin_notices', array( $this, 'explore_theme_review_notice' ), 0 );
		add_action( 'admin_init', array( $this, 'explore_ignore_theme_review_notice' ), 0 );
		add_action( 'admin_init', array( $this, 'explore_ignore_theme_review_notice_partially' ), 0 );
	 ...
	 */
	public function explore_theme_review_notice() {
		...
		<div class="notice updated theme-review-notice" style="position:relative;">
			<p>
				...
				printf(
					...
					esc_html__(
						'Howdy, %1$s! It seems that you have been using this theme for more than 15 day. We hope you are happy with everything that the theme has to offer. If you can spare a minute, please help us by leaving a 5-star review on WordPress.org.  By spreading the love, we can continue to develop new amazing features in the future, for free!', 'explore'
					),
			...
			<div class="links">
				...
				<a href="<?php echo esc_url( 'https://themegrill.com/support-forum/forum/explore-free/' ); ?>" class="btn button-secondary" target="_blank">
					<span class="dashicons dashicons-edit"></span>
					<span><?php esc_html_e( 'Got theme support question?', 'explore' ); ?></span>
				</a>
			</div>

			<a class="notice-dismiss" style="text-decoration:none;" href="?nag_explore_ignore_theme_review_notice=0"></a>
		</div>
	 ...
	 */
	public function explore_ignore_theme_review_notice() {

		$user_id  = get_current_user_id();

		/* If user clicks to ignore the notice, add that to their user meta */
		if ( isset( $_GET['nag_explore_ignore_theme_review_notice'] ) && '0' == $_GET['nag_explore_ignore_theme_review_notice'] ) {
			add_user_meta( $user_id, 'explore_ignore_theme_review_notice', 'true', true );
		}
	 ...
	 */
	public function explore_ignore_theme_review_notice_partially() {

		$user_id  = get_current_user_id();

		/* If user clicks to ignore the notice, add that to their user meta */
		if ( isset( $_GET['nag_explore_ignore_theme_review_notice_partially'] ) && '0' == $_GET['nag_explore_ignore_theme_review_notice_partially'] ) {
			update_user_meta( $user_id, 'nag_explore_ignore_theme_review_notice_partially', time() );
		}
	 ...
	 */
	public function explore_theme_rating_notice_data_remove() {
	 ...
	 */
	public function explore_theme_rating_notice_enqueue() {

		wp_enqueue_style( 'explore-theme-review-notice', get_template_directory_uri() . '/css/admin/theme-review-notice.css' );
